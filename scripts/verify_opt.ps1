...
try {
    $response = Invoke-RestMethod -Uri "http://localhost:8080/api/analyze" -Method Post -Headers $headers -Body $body
...
Invoke-RestMethod -Uri "http://localhost:8080/api/translate" -Method Post -Headers $headers -Body $bodyCache | Out-Null
$sw.Stop()
...
try {
    Invoke-RestMethod -Uri "http://localhost:8080/api/translate" -Method Post -Headers $headers -Body $bodyLong
} catch {
